...
namespace Simulation
{
    class Simulation : System.Windows.Forms.Form
    {
        ...
        {
            bool toggleFrozen = !GetKeyPressed(0x20);
            if (!toggleFrozen && lastFrozenPress)
                Frozen = !Frozen;

            lastFrozenPress = toggleFrozen;
            if (Frozen)
            {
                MouseControls();
                Cube.tick(Dt, Wall1);
        ...
        public void DrawVector(Graphics g, Color color, Vector start, Vector vector)
        {
            g.DrawLine(new Pen(color, 2), (float)start.X, (float)start.Y, (float)(vector.X + start.X), (float)(vector.Y + start.Y));
        }
        ...
        public void MouseControlsFrozen(Graphics g)
        {
            MousePosition = Vector.FromPoint(Cursor.Position);
            if (GetLeftMousePressed())
            {
        ...
        public static bool GetKeyPressed(int VirtualKeyPressed)
        {
            if (GetAsyncKeyState(VirtualKeyPressed) == 0)
                return false;
            else
                return true;
        }
